        ...
        {showRelated && showRelated.count > 0 && (
          ...
          >
            <div className="flex items-center justify-between text-xs text-gray-500 hover:text-gray-400 transition-colors">
              <span>
                {expandedGroups.has(showRelated.themeKey) ? '▾' : '▸'} {t('moreRelated', { count: showRelated.count })}
      ...
      <div className="flex items-center justify-between p-4 rounded-t-xl">
        ...
        >
          ...
          <div className="text-left">
            <h2 className="text-lg font-semibold text-white">{t('majorEventsFeed')}</h2>
            <p className="text-xs text-gray-400">
              {loading ? t('loading') : `${t('eventGroups', { count: eventGroups.length })} ${t('totalEvents', { total: events.length })}`}
            </p>
          </div>
        </button>
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="hidden sm:inline">{t('beginner')}</span>
        ...
        <div className="px-4 pb-4">
          ...
          {!loading && !error && events.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 text-xs mt-1">{t('eventsWillAppear')}</p>
            </div>
          )}
            ...
            <div className="mt-4 pt-3 border-t border-gray-700/50">
              <div className="text-xs text-gray-500 space-y-0.5">
                <div className="flex flex-wrap gap-x-4 gap-y-0.5">
                  <span>
                    {t('updatedServer')}: <span className="text-gray-400">{formatTimestamp(meta.generatedAt)}</span>
                  </span>
                  <span>
                    {t('fetchedBrowser')}: <span className="text-gray-400">{formatTimestamp(meta.fetchedAt)}</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-col items-center gap-2">
              <div className="flex gap-2 w-full">
                ...
                >
                  ...
                  {forceRefreshing ? `${t('forceRefresh')}...` : t('forceRefresh')}
                </button>
              </div>
              {lastUpdated && (
                <span className="text-xs text-gray-500">
