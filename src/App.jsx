...
// Sectors for filtering (keys for translation)
const SECTOR_KEYS = ['all', 'technology', 'finance', 'automotive', 'energy']
const SECTOR_VALUES = ['All', 'Technology', 'Finance', 'Automotive', 'Energy']
...
function SignalCard({ card, onSetAlert }) {
  ...
  // Calculate correct predictions for summary
  const correctCount = card.quarterlyHistory?.filter(q => q.accurate).length || 0
  const totalCount = card.quarterlyHistory?.length || 0
  ...
  const getResultLabel = (result) => {
    ...
    if (r === 'beat') return t('beat')
    if (r === 'miss') return t('miss')
    return t('inline')
  }
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-5 border border-gray-700 hover:border-gray-600 transition-all duration-300">
      ...
      <div className="mb-4">
        ...
        >
          <span className="text-sm text-gray-300">{t('patternHistory')} ({totalCount} {t('quarters')})</span>
          <svg
        ...
        {isHistoryOpen && (
          <div className="mt-2 space-y-1.5 max-h-64 overflow-y-auto">
            {/* Summary row */}
            <div className="p-2 bg-blue-500/10 border border-blue-500/30 rounded text-xs text-blue-300 font-medium">
              {t('summaryCorrect', { correct: correctCount, total: totalCount })}
            </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {/* Sector Filter - Only show on earnings tab */}
              {activeTab === 'earnings' && (
                <select
                ...
                >
                  {SECTOR_KEYS.map((key, index) => (
                    <option key={key} value={SECTOR_VALUES[index]}>{t(key)}</option>
                  ))}
                </select>
              )}
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* Radar Tab - Major Events */}
        {activeTab === 'radar' && (
          <MajorEventsPanel />
        )}

        {/* Earnings Tab - Pattern Cards */}
        {activeTab === 'earnings' && (
          <>
            {/* Stats Overview */}
